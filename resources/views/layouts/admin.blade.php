    ...
    <div class="flex min-h-screen">
        ...
        <aside class="hidden lg:flex w-64 flex-col fixed inset-y-0 z-30
            ...
            <div class="px-6 py-6 border-b border-ink-200 dark:border-white/10">
                <a href="{{ route('home') }}" class="flex items-center gap-2 font-bold text-lg" data-testid="admin-logo">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 text-sm">
                ...
                @foreach($menu as [$route, $label, $icon])
                    <a href="{{ route($route) }}" wire:navigate
                       data-testid="nav-{{ Str::slug($label) }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition
                              {{ request()->routeIs($route) ? 'bg-brand-500/10 text-brand-600 dark:text-brand-300 font-semibold' : 'text-ink-700 dark:text-ink-300 hover:bg-ink-100 dark:hover:bg-white/5' }}">
        ...
        <div class="lg:pl-64 flex-1 flex flex-col">
            <header class="sticky top-0 z-20 border-b border-ink-200 dark:border-white/10
                ...
                <div class="flex items-center justify-between px-6 py-4">
                    <h1 class="font-semibold">{{ $title ?? 'Admin' }}</h1>
